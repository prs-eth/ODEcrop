...
##Crop images
#target_map_image = target_map_image[2400:3009,1344:2282] 
#performance_map = performance_map[2400:3009,1344:2282,:] 

img = Image.fromarray(np.uint8(performance_map))
img.save(output_file + '_performance_' + model + '_fold_'+str(fold)+'.png')

#
##pred_map_image = pred_map_image[:int(target_map_image.shape[0]*0.95),:int(target_map_image.shape[1]*0.66)] 
##target_map_image = target_map_image[:int(target_map_image.shape[0]*0.95),:int(target_map_image.shape[1]*0.66)] 
#
#
...
#
#ax.axis("off")
#legendfig.savefig("./viz/legend_croped_fold_" +str(fold)+ ".png",dpi=600)
#
#
#legendfig, ax = plt.subplots(1, 1)
#legend_elements = [Line2D([0], [0], color=[0,182/255.,0], lw=8, label='True'), Line2D([0], [0], color=[212/255.,0,0], lw=8, label='False')]
#ax.legend(handles=legend_elements, ncol=2, loc="center", borderpad=1.2, handlelength=0.4, borderaxespad=0.1, prop=fontP, frameon=None)
#ax.axis("off")
#legendfig.savefig("./viz/cw_legend.png",dpi=600)
#
#
#
#target_map_RGB = np.ones([target_map_image.shape[0],target_map_image.shape[1],3])*255
#pred_map_RGB = np.zeros([pred_map_image.shape[0],pred_map_image.shape[1],3])
#performance_map_RGB = np.zeros([pred_map_image.shape[0],pred_map_image.shape[1],3])
#
#
...
#
#        #pred_pix_color = colordict[ label_names[labels.index(pred_pix_val)] ]
#        #pred_map_RGB[i_x,i_y,:] = np.array(pred_pix_color)
#        
#
##pred_map_image_img = Image.fromarray(np.uint8(pred_map_RGB))
##pred_map_image_img.save(output_file + '_pred_img.png')
#
#target_map_image_img = Image.fromarray(np.uint8(target_map_RGB))
#target_map_image_img.save(output_file + '_target_img_croped_fold_'+str(fold)+'.pdf')
