...
def parse_args():
    ...
    parser.add_argument('-hd', "--hidden", default=128, type=int, help="hidden dim")
    parser.add_argument('-nl', "--layer", default=2, type=int, help="num layer")    
...
def main(
    ...
    traindataset = Dataset("/scratch/tmehmet/train_set_24X24_debug.hdf5", 0.5, 'train')
    #traindataset_2 = Dataset("/scratch/tmehmet/ZH_train_patches_24x24_instance.hdf5", 0.9, 'train')
    testdataset =  Dataset_eval("/scratch/tmehmet/train_set_24X24_debug.hdf5", 0.5, 'test')    
    #testdataset =  Dataset("/scratch/tmehmet/ZH_train_patches_24x24_instance.hdf5", 0.0, 'test')  
...
#                 enc_crnn=False, enc_attn=False, enc_attn_type='None')
    ...
    lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.1, total_steps=None, epochs=30, steps_per_epoch=1, pct_start=0.3, 
...
#            evaluate(network, traindataset_2) 
            print("\n Eval on test set")
            cm = evaluate_fieldwise(network, testdataset) 
