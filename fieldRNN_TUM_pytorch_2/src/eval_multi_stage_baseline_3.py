...
def evaluate_fieldwise(model, dataset, batchsize=1, workers=0, viz=False):
    model.eval()
    ...
    if viz:
        ...
        np.savez('./viz/result_tum', targets=targets, predictions=predictions, predictions2=prediction_wo_fieldwise, cm=confusion_matrix)
...
def evaluate_fieldwise2(model, dataset, batchsize=1, workers=0, viz=False, fold_num=5, name='baseline'):
...
def evaluate_fieldwise_stage1(model, dataset, batchsize=1, workers=0, viz=False, fold_num=5):
    
    label_list_glob = dataset.label_list_glob
    label_list_local_1 = dataset.label_list_local_1
    ...
    predictions_local_1_wo_unknown = np.ones_like(predictions_wo_unknown)*7777
    targets_local_1_wo_unknown = np.ones_like(targets_wo_unknown)*7777
    
    
    for i in range(len(label_list_glob)):        
        predictions_local_1_wo_unknown[predictions_wo_unknown==label_list_glob[i]] = label_list_local_1[i]
        targets_local_1_wo_unknown[targets_wo_unknown==label_list_glob[i]] = label_list_local_1[i]

    predictions_wo_unknown = predictions_local_1_wo_unknown
    targets_wo_unknown = targets_local_1_wo_unknown
    ...
    if viz:
        ...
        np.savez('./viz/result_bigru_' + str(fold_num) , targets=targets, predictions=predictions, predictions2=prediction_wo_fieldwise, cm=confusion_matrix)
