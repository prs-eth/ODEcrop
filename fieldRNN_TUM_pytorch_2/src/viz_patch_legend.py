...
patch_list = [patch_1,patch_2,patch_3,patch_4,patch_5]

t = 0

for patch_1 in patch_list:
    model = 'baseline'
...
#        performance_map_occ = performance_map_occ[patch_1[0]:patch_1[0]+patch_size,patch_1[1]:patch_1[1]+patch_size] 
    ...
    if t == 0:
        target_map_image_concat = target_map_image
        t = 1
    else:
        target_map_image_concat = np.concatenate((target_map_image_concat, target_map_image), axis=0)
...
valid_labels = np.unique(target_map_image_concat)
...
ax.legend(handles=legend_elements, ncol=1, loc="center", borderpad=1.2, handlelength=0.4, borderaxespad=0.1, prop=fontP, frameon=None)
#ax.legend(handles=legend_elements, ncol=3, loc="center", prop=fontP, frameon=None)

ax.axis("off")
legendfig.savefig("./viz/patch/legend_patch_glob3.png",dpi=600)
...
ax.legend(handles=legend_elements, ncol=1, loc="center", borderpad=1.2, handlelength=0.4, borderaxespad=0.1, prop=fontP, frameon=None)
ax.axis("off")
legendfig.savefig("./viz/patch/cw_legend.png",dpi=600)