...
from models.network_gt import model_GT
...
def parse_args():
    ...
    parser.add_argument('-sd', "--seed", default=9999, type=int, help="random seed")
...
def main(
    ...
    optimizer_0 = torch.optim.Adam(network.parameters(), lr=lr, weight_decay=weight_decay)
    optimizer = torch.optim.Adam(network_gt.parameters(), lr=lr, weight_decay=weight_decay)
    ...
    if snapshot is not None:        
        state = resume(snapshot,model=network, optimizer=optimizer_0)
...
def train_epoch(dataloader, network, network_gt, optimizer, loss, loss_local_1, loss_local_2, loggers, lambda_1,lambda_2,lambda_0, stage):
    ...
    for iteration, data in enumerate(dataloader):
        ...
        #GT refinement -------------------------------------------------
        #output_glob_R = network_gt([output_local_1.detach(), output_local_2.detach(), output_glob.detach()])
