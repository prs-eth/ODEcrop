...
from eval_course_2 import evaluate, evaluate_fieldwise
...
def parse_args():
    ...
    parser.add_argument('-s', "--snapshot", default='./trained_models_multistage/debug_course_label_2_model.pth', type=str, help="load weights from snapshot")
    ...
    parser.add_argument('-lrs', "--lrSC", default=1, type=int, help="lrScheduler")    
    parser.add_argument('-nm', "--name", default='debug_course_label_2', type=str, help="name")
...
def main(
    ...
    traindataset = Dataset("/scratch/tmehmet/train_set_24X24_debug.hdf5", 0., 'train')
    testdataset =  Dataset_eval("/scratch/tmehmet/train_set_24X24_debug.hdf5", 0., 'test')    
    
    nclasses = traindataset.n_classes
    #nclasses = 125
    print('Num classes:' , nclasses)
    LOSS_WEIGHT  = torch.zeros(nclasses)
    LOSS_WEIGHT[1:10] = 1
