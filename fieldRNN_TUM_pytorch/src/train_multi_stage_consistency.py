...
import os
#from eval_multistage import evaluate, evaluate_fieldwise
...
def parse_args():
    ...
    parser.add_argument('-c', "--checkpoint_dir", default='trained_models_multistage_consistency', type=str, help="directory to save checkpoints")
    ...
    parser.add_argument('-nm', "--name", default='debug_multi_consistency', type=str, help="name")
...
def consistency_loss(output_glob, output_local_1, l1_2_g):
    local1_exp = torch.zeros_like(output_glob)
    for i in range(output_glob.shape[1]):
        local1_exp[:,i] = output_local_1[:,int(l1_2_g[i])]
        
    loss = torch.mean( torch.max( torch.zeros_like(output_glob), output_glob - local1_exp)**2 )
    return loss
...
def main(
    ...
    #for Consistancy losses
    l1_2_g = traindataset.l1_2_g
    l2_2_g = traindataset.l2_2_g
    l1_2_l2 = traindataset.l1_2_l2
    ...
    #Define the model
    network = multistageSTARSequentialEncoder(24,24, nstage=3, nclasses=nclasses, nclasses_l1=nclasses_local_1, nclasses_l2=nclasses_local_2, 
                                              input_dim=4, hidden_dim=hidden, n_layers=layer)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train_epoch(traindataloader, network, optimizer, loss, loss_local_1, loss_local_2, 
                    ...
                    l1_2_g=l1_2_g, l2_2_g=l2_2_g, l1_2_l2=l1_2_l2)
#        print("\ntest")
...
def train_epoch(dataloader, network, optimizer, loss, loss_local_1, loss_local_2, loggers, lambda_1,lambda_2,lambda_0, stage, l1_2_g, l2_2_g, l1_2_l2):
    ...
    for iteration, data in enumerate(dataloader):
        ...
        #Consistency losses
        c_loss_1 = consistency_loss(output_glob, output_local_1, l1_2_g)
        c_loss_2 = consistency_loss(output_glob, output_local_2, l2_2_g)
        c_loss_3 = consistency_loss(output_local_2, output_local_1, l1_2_l2)

        total_loss += c_loss_1 + c_loss_2 + c_loss_3
        ...
        mean_loss_c_1 += c_loss_1.data.cpu().numpy()
        mean_loss_c_2 += c_loss_2.data.cpu().numpy()
        mean_loss_c_3 += c_loss_3.data.cpu().numpy()
        
#        print('-'*10)
#        print(c_loss_1.data.cpu().numpy())
#        print(c_loss_2.data.cpu().numpy())
#        print(c_loss_3.data.cpu().numpy())
    ...
    print('Consistency Loss -1 : %.4f'%(mean_loss_c_1/iteration))
    print('Consistency Loss -2 : %.4f'%(mean_loss_c_2/iteration))
    print('Consistency Loss -3 : %.4f'%(mean_loss_c_3/iteration))
