...
def parse_args():
    ...
    parser.add_argument('-trt', "--tr_type", default=2, type=int, help="transformer type")
...
def main(
    ...
    traindataset = Dataset(data_file, 0., 'train', False, fold_num, gt_path, time_downsample_factor=4)
    testdataset =  Dataset(data_file , 0., 'test', True, fold_num, gt_path, time_downsample_factor=4)    
    ...
    #Define the model
    network = multistageSTARSequentialEncoder(24,24, nstage=stage, nclasses=nclasses, nclasses_l1=nclasses_local_1, 
                                              nclasses_l2=nclasses_local_2, input_dim=4, hidden_dim=hidden, n_layers=layer, wo_softmax=True)
    network_gt = model_GT(nclasses=nclasses, s1_2_s3=s1_2_s3, s2_2_s3=s2_2_s3, transformer_type=transformer_type)
...
def train_epoch(dataloader, network, network_gt, optimizer, loss, loss_local_1, loss_local_2, loggers, lambda_1,lambda_2,lambda_0, 
                stage, grad_clip, transformer_type=1):
    ...
    mean_loss_local_2 = 0.
    mean_loss_gt = 0.
    mean_loss_local_1_gt = 0.
    mean_loss_local_2_gt = 0.
    ...
    for iteration, data in enumerate(dataloader):
        ...
        l_glob = loss( F.log_softmax(output_glob, dim=1), target_glob)
        l_local_1 = loss_local_1( F.log_softmax(output_local_1, dim=1), target_local_1) 
        l_local_2 = loss_local_2( F.log_softmax(output_local_2, dim=1), target_local_2) 
        ...
        if transformer_type==2:
            output_glob_R,  output_local_1_R, output_local_2_R = network_gt([output_local_1, output_local_2, output_glob])
            l_gt = loss(output_glob_R, target_glob.contiguous().view(-1))
            l_local_1_gt = loss_local_1(output_local_1_R, target_local_1.contiguous().view(-1))
            l_local_2_gt = loss_local_2(output_local_2_R, target_local_2.contiguous().view(-1))
            
            mean_loss_gt += l_gt.data.cpu().numpy()
            mean_loss_local_1_gt += l_local_1_gt.data.cpu().numpy()
            mean_loss_local_2_gt += l_local_2_gt.data.cpu().numpy()

            l_total_gt = l_gt + lambda_1 * l_local_1_gt + lambda_2 * l_local_2_gt
        else:
            output_glob_R = network_gt([output_local_1, output_local_2, output_glob])
            l_total_gt = loss(output_glob_R, target_glob.contiguous().view(-1))
            
            mean_loss_gt += l_total_gt.data.cpu().numpy()

        #GT refinement -------------------------------------------------
        
        total_loss = total_loss + l_total_gt
        total_loss.backward()
    ...
    print('-'*5)
    if transformer_type == 2:
        print('Local Loss 1 - Refined: %.4f'%(mean_loss_local_1_gt/iteration))
        print('Local Loss 2 - Refined: %.4f'%(mean_loss_local_2_gt/iteration))        
