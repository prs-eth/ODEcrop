...
def test2(model, model_gt, dataloader):
    model.eval()
    ...
    for iteration, data in tqdm(enumerate(dataloader)):
        ...
        z3_refined, _, _ = model_gt([z1.detach(), z2.detach(), z3.detach()])
...
def evaluate_fieldwise(model, model_gt, dataset, batchsize=1, workers=0):

    dataloader = torch.utils.data.DataLoader(dataset=dataset, batch_size=batchsize, num_workers=workers)

    logprobabilites, targets, gt_instance, logprobabilites_refined = test2(model, model_gt, dataloader)
    predictions = logprobabilites.argmax(1)
    ...
    #targets_refined_wo_unknown = targets_refined[valid_crop_samples]
