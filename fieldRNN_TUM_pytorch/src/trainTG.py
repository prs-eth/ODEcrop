...
def parse_args():
    ...
    parser.add_argument('-c', "--checkpoint_dir", default='trained_models_TG', type=str, help="directory to save checkpoints")
...
def main(
    ...
    traindataset = Dataset("/cluster/scratch/tmehmet/TG_expYear19.hdf5", 0.5, 'all')
    traindataset_2 = Dataset_eval("/cluster/scratch/tmehmet/train_set_24X24_debug.hdf5", 0.66, 'train')
    testdataset =  Dataset_eval("/cluster/scratch/tmehmet/TG_expYear18.hdf5", 0.5, 'all')    
...
#        test_epoch(testdataloader, network,loss, loggers=(logger, vizlogger))
        ...
        if epoch>-0 and epoch%1 == 0:
            print("\n Eval on train set")
            evaluate_fieldwise(network, traindataset_2, batchsize=1) 
            print("\n Eval on test set")
            test_acc = evaluate_fieldwise(network, testdataset, batchsize=1) 
