...
import os
from eval_MSconvSTAR_ablation import  evaluate_fieldwise_full_coverage as evaluate_fieldwise
...
def parse_args():
    ...
    parser.add_argument('-s', "--snapshot", default='/home/pf/pfstaff/projects/ozgur_deep_filed/multi_stage/trained_models_multistage_transformer/debug_TRANSFORMER_4_0.001_6_64_2_0.1_0.5_0.0001_2_labelsC.csv_0_model.pth', type=str, help="load weights from snapshot")
    ...
    parser.add_argument('-fd', "--fold", default=2, type=int, help="5 fold")   
...
def main(
    ...
    coverage = np.zeros([len(ps),3,2])
    
    for i in range(len(ps)):
        c, a = evaluate_fieldwise(network, network_gt,testdataset, batchsize=batchsize, viz=False, fold_num=fold_num , p=ps[i]) 
        
        coverage[i,:,0] = c
        coverage[i,:,1]  = a
    
    
    np.save('./ablation/full_coverage_' + str(fold_num) , coverage)
